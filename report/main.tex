...
\begin{abstract}
    Antibiotic resistance in \textit{Streptococcus pneumoniae} is a growing reason of concern in public health. In this paper, we investigate the dynamics of antibiotic sensitivity of \textit{S. pneumoniae} in response to vaccination. In a simple model of antibiotic resistance, we find that a change in availability of hosts leads to a transient faster increase of sensitive strains with respect to resistant ones. This faster increase is only visible in the early phase of the disease propagation. As an extension to this framework, we set up another model which incorporates vaccination explicitly. This new model supports our prediction of transient dominance of sensitive strains. The dynamics of the model can imply that during a vaccination campaign a shift towards a higher infectiousness of \textit{S. pneumoniae} is to be expected. Data from the Massachusetts pneumococcal vaccination campaign does not show the same trends as we predict with our models. This means that i) the data resolution is too poor and we could not see the trends we expect to see or ii) the model misses important aspects of vaccination. 
\end{abstract}
...
Parameter    & Description    & Simulation rate [month$^{-1}$]                                  \\ \hline
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{figures/SIS_numerical.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{figures/phase_plane.png}
...
Through the SIS antibiotic resistance model in equations \ref{eq:SIS}, we get an idea about how the sensitive and resistant strains change in frequency subject to a decrease in the available hosts (figure \ref{fig:SIS_numerical}). Vaccination is a process that leads to a change in the availability of hosts, so we expect a transient faster increase of sensitive in a vaccination setting. Depending on the parameterisation this can be followed by domination of resistant types at equilibrium. Via the phase plane analysis in figure \ref{fig:phase_plane}, we understand that initial conditions matter a lot for transient phenomena. The flaw in the modelling up until now is, that we do not account for vaccination explicitly. This leads to the next, more complex SISV model.
...
\frac{dI_r^{v,S}}{dt} &= \frac{\beta}{c_{\beta}} S I_r^{v,S} - c_{\gamma}\gamma I_r^{v,S} - \nu I_r^{v,S}\\
...
In the SIS model trajectories in figure \ref{fig:SIS_numerical} we saw a transient dominance of sensitive types of resistant types before going to equilibrium. We see a similar behaviour in figure \ref{fig:SISV_numerical} (A). The parameterisation in both cases was the same. The difference in the SISV model is the explicit vaccination. The vaccination starts at $t=50$ and is characterised by a drop in susceptibles for the vaccine-types and a steep rise in vaccinated individuals (susceptibles for the non-vaccine-types). We observe an overall increase in the non-vaccine-types. In a transient phase, the sensitive non-vaccine-types increase faster than the resistant types and dominate the resistant types over $\sim 50$ months. This is again a form of trade-off as the transient decrease in susceptibles for the vaccine type and the increase in vaccinated individuals (susceptibles for the non-vaccine-types) change the context of competition. This leads to a transient dominance of sensitives.
...
The simulation gives rise to the hypothesis that a high level of susceptible hosts for vaccine types followed by a decrease, as is the case during an ongoing vaccination campaign, will lead to a transient increase of sensitive frequencies for non-vaccine-types. This increase is faster than the increase in resistant strains. To verify this claim of transient dominance, we look at data from a pneumococcal vaccination campaign.

\subsection{Data Analysis}

The dataset is from Massachusetts hospitals, collected in the form of nasal swaps from children. First, we look at the development of vaccine-types vs. non-vaccine-types over time. This is visible in figure \ref{fig:subplot_freq} (A). The vaccination campaign with PCV7 started in 2000. Therefore, we assume that the year 2001 constitutes as a pre-vaccination data point. This pre-vaccination data point tells us that the non-vaccine-types are already more common than the vaccine-types in 2001. We can manually fit our model in \ref{fig:SISV_numerical} (A) to this and set the parameter values as indicated. This gives us a similar relative frequency in \ref{fig:SISV_numerical} (B) at the start of vaccination. The question that arises from this is, whether these dynamics are only visible for the chosen parameter values. For a broader sensitivity analysis of the parameters refer to appendix \ref{parameterisation}.
Next, it is visible in figure \ref{fig:subplot_freq} (A) that first, the vaccine-type frequencies decline before the non-vaccine-type frequencies rise. In the simulation figure \ref{fig:SISV_numerical} (A) we observe that the vaccine-types are replaced by non-vaccine-types but without a lag.\\
...
In the results section on the SIS section we have seen that the drop in susceptibles early in the simulation leads to interesting transient dynamics. This drop in susceptibles is what we expect to happen via vaccination as well in form of a drop in susceptibles  for the vaccine-types. The vaccination campaign will lead to a drop in fully-susceptible individuals and an increase in vaccinated individuals. This means we expect similar processes to happen in vaccination as in the SIS model after the drop of susceptibles.\\
...
\section{Acknowledgments} I thank Sonja Lehtinen for the kind supervision, it has been a very pleasant working atmosphere and a stimulating research topic. I thank Sebastian Bonhoeffer for enabling the lab rotation in the theoretical biology group.
